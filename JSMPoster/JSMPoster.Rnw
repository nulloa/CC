...
\author{Nehemias Ulloa$^{1}$\\ [0.5ex]
        Frederick O. Lorenz$^{1}$} % Author(s)
...
The overall theme of this poster will be polynomial regression. What I hope you take away from this poster is:
\begin{itemize}
\item Difference scores can be a problem
\item What is polynomial regression?
\item Recreate past examples
\item Apply it to our own data
\begin{itemize}
...
\begin{itemize}
\item Difference scores are used to see how well two variables or indices 'fit' together. Some of the most common difference scores use the algebraic, absolute, and squared difference between the two variables ($X$ and $Y$) as shown by Equations \ref{eq:diffscore}, \ref{eq:absdiffscore}, and \ref{eq:squarediffscore} respectively. These difference scores are often used to predict for an outcome variable ($Z$) as shown in the equations below,
\item{
\begin{align}
