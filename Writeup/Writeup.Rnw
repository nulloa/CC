...
\lhead{}
\chead{Applying Polynomial Regression to Dyadic Data}
\rhead{}
\lfoot{}
\cfoot{\thepage}
\rfoot{}
...
\HRule \\[0.4cm]
{ \huge \bfseries Applying Polynomial Regression to Dyadic Data}\\[0.4cm] % Title of your document
\HRule \\[1.5cm]
...
\emph{Author:}\\
Nehemias \textsc{Ulloa} % Your name
\end{flushleft}
\end{minipage}
~
\begin{minipage}{0.4\textwidth}
...
\emph{Advisor:} \\
Dr. Frederick O. \textsc{Lorenz} % Supervisor's Name
\end{flushright}
\end{minipage}\\[4cm]
...
%------------------------------------%
\section{The Model}
The linear regression model (Eq \ref{eq:linreg}) is familiar to most researchers; it is often used in research due in part to the ease and familiarity of the interpretations of the parameters and the ease of computation. A slight tweak to simple linear models is to include two explanatory variables as in Eq \ref{eq:linreg2}. We call Eq \ref{eq:linreg2} the linear difference model; this particular model does not contain an interaction nor a separate parameter for each variable. The lack of parameters is what leads to issues regarding interpret-ability of the difference. It is difficult to tease out which explanatory variable is causing more change to the response when you are only considering the difference of the two variables.
...
\subsubsection{Stationary Point: How and Why?}
As we hinted to earlier, polynomial regression draws on ideas from response surface methodology, starting with the stationary point. The stationary point is the points on the $X$ and the $Y$ - axes where the slope of the response ($Z$) is zero no matter which direction you take the derivative. This corresponds to find the minimum and maximum of a function from calculus; it is a similar concept except now we have a three dimensional surface rather than just a two-dimensional surface. We are interested in the stationary point because they allow us to see which values of our explanatory variables provide the ``best'' fit for the response (``best'' meaning minimum or maximum). The benefit of knowing the stationary point is it gives us a goal to reach or not reach depending on the situation. (We will see examples of the usefulness of the stationary point later.) Using our intuition from calculus, we take the derivatives of Equation \ref{fittedmod} with respect to $x$ and $y$, set the derivatives equal to zero, and solve for $x$ and $y$ in terms of {\bf b} and {\bf B} to find the stationary point (we'll refer to these points as $x_0$ and $y_0$). \\

To find the stationary point, let's begin with taking the partial derivatives with respect to $x$ and $y$
\begin{align*}
\left[ \begin{array}{cc}
...
So our the stationary point is 
\begin{align}
...
Using our output from the $\verb|lm|$ function noted earlier in $\verb|R|$, we can get the stationary point. Note we can use other computing programs as well as $\verb|R|$ (e.g. $\verb|SAS|$, $\verb|MPlus|$, etc.) The $\verb|R|$ code is as follows:
...
\subsubsection{Predicted Response at the Stationary Point}
Once we have the stationary point, we can get the predicted responses at the stationary point by simply plugging in the stationary point into Equation \ref{fittedmod}. The predicted response at the stationary point give us the optimized value for the response variable i.e. where the response surface reaches its maximum or minimum. In matrix notation, the predicted response is given as:
...
Now we have transformed our principal axes back to functions of our original variables. This will make them easier to plot and interpret. In a concave surface, the first principal axis (Eq \ref{eq:principalaxes1}) tells us where the surface has the lowest downward curvature. This is where the two explanatory variables create a maximized surface that decreases the least as we move along the line. The second principal axis is the opposite of the first principal axis. The second principal axis is the line in the surface where the outcomes decreases the most. It is the line in the surface where we the two explanatory variables create the greatest decrease in the response. The simplest case would be when the first principal axis is parallel to the line in the surface where $X=Y$ and the second principal axis is parallel to the line in the surface where $X=-Y$. If these axes are parallel to these lines, then our intuition about our explanatory variables and their congruence, or lack of, would be correct. \\
...
Z_{2,i} &- \text{physician's perceived agreement with patient } i  \text{ on the illness treatment} \\
...
d <- read.spss("../../ccData/phillipsData/Congruence PRIM.sav", to.data.frame=TRUE)
...
\text{Unconstrained Abs Difference Model: } &Z = \beta_0 + \beta_1 X + \beta_2 Y + \beta_3 W + \beta_4 WX + \beta_5 WY + \epsilon \label{eq:unconabs} \\
...
We can use our functions to get the stationary point and the principal axes.
<<Phillips_Stat_Principal, echo=TRUE>>=
statpts(lm2Square)
paxis(lm2Square)
@

So we can see that our stationary point is at $(\Sexpr{round(statpts(lm2Square)$x0,2)}, \Sexpr{round(statpts(lm2Square)$y0,2)})$. 
...
Set 1:
\begin{align*}
Z &: \text{Observed Relationship Quality of couple } i \text{ at Time Wave 0} \\
Y_1 &: \text{Wife's Report of Husband's Hostility for couple } i \text{ at Time Wave 0} \\
X_1 &: \text{Husband's Report of Husband's Hostility for couple } i \text{ at Time Wave 0} \\
\end{align*}
...
\begin{table}[htp]
\caption{This table follows the same format as Table \ref{tab:PhillipsTabReplication}, in that it summarizes the confirmatory approach to polynomial regression. Here we see that the   where $* p <0.05$; $** p <0.01$; $*** p <.001$}
\begin{center}
...
Just as we did earlier, we will use our functions to get the stationary point and the principal axes.
...
The stationary point is located at $(\Sexpr{round(statpts(lm2Square)$x0,2)}, \Sexpr{round(statpts(lm2Square)$y0,2)})$; due to our convex nature of the surface, these points are the levels of relationship instability that will minimize the relationship quality in a marriage. The predicted response at the stationary point is $\Sexpr{round(predstatpts, 3)}$ which is low but not too much. It is interesting that relationship quality seems to be minimized when the wife's perceived relationship instability is really low while the husbands perceived relationship instability is on the higher side.
