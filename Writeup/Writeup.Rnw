...
One of the most common methods in analyzing dyadic data is difference scores. Difference scores are used to see how well two variables/components 'fit' together. Some of the most common difference scores use the algebraic, absolute, and squared difference between the two variables as shown by Equations \ref{eq:diffscore}, \ref{eq:absdiffscore}, and \ref{eq:squarediffscore} respectively. These difference scores are often modeled as a predicts for an outcome variable labeled $Z$ in the equations below,
...
The problem is there are many methodological issues with difference scores. Dr. Edwards describes many of these issues in his paper \emph{Problems with the Use of Profile Similarity Indices in the Study of Congruence in Organizational Research} (1993) \cite{Edwards_Prob_Diff_Scores_93}. The first reason may be the most important in terms of the reason why statisticians model data; difference scores make it difficult to identify the underlying mechanism. This happens because in using difference scores, two distinct measures are turned into one entity so it becomes difficult to tease out the individual effects. These individual effects are what we interested in. Difference scores fail to tell the researcher which variables contribute the most to the response. We also lose information when using difference scores. There are assumptions used with difference scores namely, that the difference between two entities to the outcome symmetric and that the outcome is constant at all points where the two entities are equal, but to verify the assumptions we need to look at the absolute level of both measures and the direction. \\

Dr. Edwards also addressed these issues in his paper, \emph{On the Use of Polynomial Regression Equations as an Alternative to Difference Scores in Organizational Research},  and suggested using "polynomial regression equations containing the...[two variables]...composing the difference and higher-order terms."\cite{Edwards_PRAlt_93} as an alternative to difference scores. An example would be to take the two predictor variables $(X,Y)$, their squares $(X^2,Y^2)$, and their products $(XY,X^2Y,XY^2)$. A successful application of such methodology (i.e. polynomial regression) is presented by Phillips, Diefenbach, Kronish, Negron, and Horowitz (2014) in their paper titled \textit{The Necessity-Concerns Framework: a Multidimensional Theory Benefits from Multidimensional Analysis}\cite{Phillips2014}. They use polynomial regression to assess stroke survivors' concerns about medication and their necessity beliefs effect on their adherence to medication whose goal is stroke prevention. Before looking at the polynomial model Edwards proposed, we will look at some componants which are not common to normal linear regression: the model form, stationarity points, the principal axis, and the predictive surface (which we will look through 3D graphs). Polynomial regression equations avoid many of the issues with difference scores, but they do have their own set of problems. Mainly these problems have to do with interpreting the estimates of the model and interpreting the principal axis. \\
...
%------------------------------------%
\section{The Model}
The linear regression model (eq \ref{eq:linreg}) is a topic familiar to most researchers; often used in research, it is very popular due in part to the ease and familiarity of the interpretations of the parameters and the ease of computation.
...
An extension of the linear regression model is the quadratic regression model. It's form is similar to the form of a linear regression model with the addition of a quadratic term of the explanatory variable. It is very similar to the linear model in terms of computation and ease of interpret-ability. A slight tweak to simple linear models is to include two explanatory variables e.g. Eq \label{eq:linreg2}. Often these models also include their interactions e.g $z = a + bx + cy + dxy$. In this project, we consider a composition of these two ideas; a polynomial(quadratic) regression using two predictors and the interaction (Eq \ref{eq:quadreg}). We will be using the basic framework of response methodology to look at stationary points and principal axes. Using the response surface methodology will allow us to take advantage of some key properties developed by that field. So looking how to develop the stationary points and principal axes will be key because they will be very useful in making inference on the predicted surface. We are going to demonstrate how this framework has been used in a few papers and then we will apply these methods our own data.


%---------- Model Form --------------%
\subsection{Form}
Let us consider a model with 2 explanatory variables and a single response. We can write the model as follows:
\begin{align}
...
\end{align}
then the fitted model in matrix notation:
\begin{align}
...
One of the features of the response surface we are interested in are the stationary points.  The stationary points are the points where the slope is zero no matter which direction you take the derivative. Recall the finding the minimum and maximum of a function from calculus; it is a similar concept except now we have a three dimensional surface rather than just a two-dimensional surface. We are interested in the stationarity points because they allow us to see which values of our explanatory variables provide the ``best'' fit for the response (``best'' meaning minimum or maximum). Using our intuition from our calculus course, we are going to take the derivatives of Equation \ref{fittedmod} with respect to $x$ and $y$, set the derivatives equal to zero, and solve for $x$ and $y$ in terms of {\bf b} and {\bf B} to find the stationarity points (we'll refer to these points as $x_0$ and $y_0$). \\
...
& = {\bf b} + 2 {\bf B d} \\
\end{align*}
Next, we set the derivatives to zero and solve for ${\bf d_0}$
\begin{align*}
...
So our stationary points are $x_0 = \frac{b_2 b_4 - 2 b_5 b_1}{4 b_5 b_3 - b_4^2}$ and $y_0 = \frac{b_1 b_4 + 2 b_2 b_3}{4 b_5 b_3 - b_4^2}$. These points represent the values in our predictors that will optimize our response. Optimizing the response either means a minimum or maximum depending on the surface shape.\\

Using our output from the $\verb|lm|$ function noted earlier in $\verb|R|$, we can get the stationary points. Note we can use other cumputing programs as well as $\verb|R|$ (e.g. $\verb|SAS|$, $\verb|MPlus|$, etc.) using the formulas.
...
We can then get the predicted responses at the stationary points by simply plugging in the stationary points into Equation \ref{fittedmod}. The predicted response at the stationary points give us the optimized value for the response variable i.e. where the response surface gets achieves its maximum or minimum. In matrix notation, the predicted response is given as:
...
The purpose of principal axes is to rotate the axes so that we can remove all the cross-product forms which will make interpretations of the model easier. Khuri and Cornell (1996), {\it Response Surfaces: Designs and Analyses} \cite{khuri1996response}, lays out the process of deriving the canonical equations. The canonical equations come from transforming the response system to its canonical form; this process occurs because they it better represents the response around the stationarity points. These canonical equations are then used in finding the principal axes. We will focus on the process of transforming these canonical equations back to functions of the original variables i.e. transform our principal axes back to the original variables.\\


To solve for principal axes, we need to first solve for eigenvalues. The equations for solving for the eigenvalues comes from Khuri and Cornell (1996) and is as follows:
...
\end{align*}
and $\lambda_2$ be the first eigen value:
\begin{align*}
...
\quad &= b_4 \div 2b_5 - b_3 - b_5 - \sqrt{(b_5 - b_3)^2 + b_4^2} \\
...
\rightarrow  y &= -P_{21}x + P_{20}
\end{align*}

where $P_{20} = y_0 + P_{21} x_0$ and $P_{21} = \frac{b_5 - b_3 - \sqrt{(b_5 - b_3)^2 + b_4^2}}{b_4}$. \\

Now we have transformed our principal axes back to functions of our original variables. This will make them easier to plot and interpret. In a concave surface, the first principal axis tells us where the surface has the lowest downward curvature. This is basically where the two explanatory variables create a maximized surface that doesn't decrease that much as we move along the line. The second principal axis is the opposite of the first principal axis. The second principal axis is the line in the surface where the outcomes decreases the most. It is the line in the surface where we the two explanotry variables create the quickest decrease in the response. Ideally, the first principal axis is parallel to the line in the surface where $X=Y$ and the second principal axis is parallel to the line in the surface where $X=-Y$. Generally, we would expect when the variables agree ($X=Y$) that the response would be good, and when they do not agree, we would expect the reponse to behave poorly. If these axes are parallel to these lines, then our intuition about our explanatory variables and their congruence, or lack of, would be correct. \\

We can easily create a function in $\verb|R|$ to take the $\verb|lm|$ output to get the principal axis:
...
\section{Recreation of Previous Results}
To check our understanding of how to run the model and make sure that our results are consistent with previous works, we reproduced the results in Phillips et al. (2012), {\it Congruence research in behavioral medicine: methodological review and demonstration of alternative methodology} \cite{Phillips2012}, where they looked at patient-reported adherence a month after the Dr. visit and its correspondence to the physician's rating of patient's health and the physician's estimate of how the patient would rate of patient's own health. We'll begin by first outlining the variables used here:
...
Presented in the paper are two different demonstrations using polynomial regression: Confirmatory and Exploratory. First discussed is the use of polynomial regression in a confirmatory approach. The assumptions of normality were checked for all the models namely looking for points with high leverage and potential influential points. No points showed any major signs of concern although there was one point which consistently had a higher leverage and Cook's D value compared to the rest of the points in the data set. This is not a major issue, but rather something we need to keep in the back of our mind and be aware of while the analysis is continued. Table \ref{tab:PhillipsTabReplication} replicates Table 2 found in Phillips et al. (2012). The idea behind the confirmatory approach is to assess the four criterion required to support a difference score or the absolute value difference score. The four criterion are:

\begin{enumerate}
\item{Check whether the unconstrained model explained a significant amount of the variance in the data.}
...
\item{Check whether if the unconstrained model is significantly better than the constrained model.}
\item{Check whether if a higher order model i.e. polynomial regression is significantly better than the constrained model.}
\end{enumerate}

We can see from Table \ref{tab:PhillipsTabReplication} when we compare to the algebraic difference score that under the unconstrained model both outcomes explained a significant amount of variation in the data; so criterion 1 was satisfied for both outcomes. However, criterion 2 was only satisfied for the outcome dealing with patient adherence as the regression coefficients for physician perceptions were not going in opposite directions and were not both significant. Criterion 3 and 4 were only met for the second outcome i.e. the physician perceptions. So by these criterions, we may consider using polynomial regression for the outcome dealing with physician perceptions while it looks as if the model using patient adherence as its outcome will be sufficiently explained using an algebraic difference score model. \\

We also checked the criterion using the absolute-value difference score model. Again we can refer to Table \ref{tab:PhillipsTabReplication} when checking the criterion. Like with the difference score model, the unconstrained model explained a significant amount of the variation for both outcomes. Criterion 2 was satisfied for both outcomes. The unconstrained model using the outcome regarding physician perception was the only unconstrained model that explained the variation in the data better then the absolute-value difference score model, but it reversed for the test to see if a higher-order polynomial model(in this case a qudratic model) to explain the variation in the data better. We can conclude that the absolute-value difference score model was not an adequate fit for either of the outcome variables, the difference score is an adequate model for the outcome concerning patient adherence, and that the polynomial regression would be the best model for the data using the physician perception outcome.

<<Phillips_Data_Setup, echo=FALSE, warning=FALSE, message=FALSE>>=
d <- read.spss("~/Dropbox/Grad Stat Classes/CC Research/CurrentData/Phillips_Data/Congruence PRIM.sav", to.data.frame=TRUE)
...
\begin{table}[htp]
\caption{This table is a replication of the Table 2 in Phillips et al. (2012) with the exception that we are only looking at the outcome regarding patient adherence where $* p <0.05$; $** p <0.01$; $*** p <.001$}
\begin{center}
...
Outcome                                   & \multicolumn{2}{l}{Constrained Model}          &  & \multicolumn{6}{l}{Unconstrained Model}                                                                                                  &  & $F_c$                                    & $F_h$                                          \\ \cline{2-3} \cline{5-10}
                                          & $(X-Y)$                    & $R^2$               &  & $X$                     & $Y$                     &                       &                       &                       & $R^2$            &  &                                          &                                                \\
\hline
Patient Adherence                         & $\Sexpr{lmDiffcoef[2]}$  & $\Sexpr{lmDiffR2}$  &  & $\Sexpr{lmcoef[2]}$   & $\Sexpr{lmcoef[3]}$   &                       &                       &                       & $\Sexpr{lmR2}$   &  & $\Sexpr{anova(lm, lmDiff)$F[2]}$   & $\Sexpr{anova(lmSquare, lmDiff)$F[2]}$   \\
Physician perception of patient agreement & $\Sexpr{lm2Diffcoef[2]}$ & $\Sexpr{lm2DiffR2}$ &  & $\Sexpr{lm2coef[2]}$  & $\Sexpr{lm2coef[2]}$  &                       &                       &                       & $\Sexpr{lm2R2}$  &  & $\Sexpr{anova(lm2, lm2Diff)$F[2]}$ & $\Sexpr{anova(lm2Square, lm2Diff)$F[2]}$ \\
\hline
                                          & $|X-Y|$                    & $R^2$               &  & $X$                     & $Y$                     & $W$                     & $WX$                    & $WY$                    & $R^2$            &  &                                          &                                                \\
\hline
Patient Adherence                         & $\Sexpr{lmAbscoef[2]}$   & $\Sexpr{lmAbsR2}$   &  & $\Sexpr{lmWcoef[2]}$  & $\Sexpr{lmWcoef[3]}$  & $\Sexpr{lmWcoef[4]}$  & $\Sexpr{lmWcoef[5]}$  & $\Sexpr{lmWcoef[6]}$  & $\Sexpr{lmWR2}$  &  & $\Sexpr{anova(lmAbs, lmW)$F[2]}$   & $\Sexpr{anova(lmAbs, lmWSquare)$F[2]}$   \\
Physician perception of patient agreement & $\Sexpr{lm2Abscoef[2]}$  & $\Sexpr{lm2AbsR2}$  &  & $\Sexpr{lm2Wcoef[2]}$ & $\Sexpr{lm2Wcoef[3]}$ & $\Sexpr{lm2Wcoef[4]}$ & $\Sexpr{lm2Wcoef[5]}$ & $\Sexpr{lm2Wcoef[6]}$ & $\Sexpr{lm2WR2}$ &  & $\Sexpr{anova(lm2Abs, lm2W)$F[2]}$ & $\Sexpr{anova(lm2Abs, lm2WSquare)$F[2]}$ \\
\hline
\end{tabular}}
\end{center}
\end{table}
...
As we can see in the figures above, the first principal axes is the axis that makes the most sense. It's support lies in the plane where we have data. As we can imagine, as the two predictor variables increase the response increases as well. This makes sense because when the physician's rating of patient's health and the physician's estimate of how patient would rate their own health agree i.e. the patient and physician are in agreement about the situation, then the patient is more likely to follow the doctor's orders in the month following the visit. We can also see this in the surface plot. Notice that the lowest points in the surface are when the physician rating and the physician's perception of the patient's own rating are at odds with each other. The surface allows us to see how the two predictor variables intereact and affect the response.
...
\begin{align*}
Z &: \text{Observed Relationship Quility of couple } i \text{ at Time Wave 0} \\
Y &: \text{Wife's Report of Relationship Instability of couple } i \text{ at Time Wave 0} \\
X &: \text{Husband's Report of Relationship Instability of couple } i \text{ at Time Wave 0} \\
\end{align*}


Just like in Dr. Phillips' analysis, we also centered our variables to ease in interpretation and analysis. We will also check to see if the use of polynomial regression is justified in our case. To do this, we will check the same criterion as we did earlier, namely:
\begin{enumerate}
\item{Whether the unconstrained model explained a significant amount of the variance in the data.}
\item{Whether the regression coefficients for the unconstrained model are in expected directions and whether they are significant to the model.}
\item{Whether if the unconstrained model is significantly better than the constrained model.}
\item{Whether if a higher order model i.e. polynomial regression is significantly better than the constrained model.}
\end{enumerate}
...
Outcome                                   & \multicolumn{2}{l}{Constrained Model}          &  & \multicolumn{6}{l}{Unconstrained Model}                                                                                                  &  & $F_c$                                    & $F_h$                                          \\ \cline{2-3} \cline{5-10}
                                          & $(X-Y)$                    & $R^2$               &  & $X$                     & $Y$                     &                       &                       &                       & $R^2$            &  &                                          &                                                \\
\hline
Relationship Quality w/ Hostility                        & $\Sexpr{lmDiffcoef[2]}$  & $\Sexpr{lmDiffR2}$  &  & $\Sexpr{lmcoef[2]}$   & $\Sexpr{lmcoef[3]}$   &                       &                       &                       & $\Sexpr{lmR2}$   &  & $\Sexpr{anova(lm, lmDiff)$F[2]}$   & $\Sexpr{anova(lmSquare, lmDiff)$F[2]}$   \\
Relationship Quality w/ Instability & $\Sexpr{lm2Diffcoef[2]}$ & $\Sexpr{lm2DiffR2}$ &  & $\Sexpr{lm2coef[2]}$  & $\Sexpr{lm2coef[2]}$  &                       &                       &                       & $\Sexpr{lm2R2}$  &  & $\Sexpr{anova(lm2, lm2Diff)$F[2]}$ & $\Sexpr{anova(lm2Square, lm2Diff)$F[2]}$ \\
\hline
                                          & $|X-Y|$                    & $R^2$               &  & $X$                     & $Y$                     & $W$                     & $WX$                    & $WY$                    & $R^2$            &  &                                          &                                                \\
\hline
Patient Adherence                         & $\Sexpr{lmAbscoef[2]}$   & $\Sexpr{lmAbsR2}$   &  & $\Sexpr{lmWcoef[2]}$  & $\Sexpr{lmWcoef[3]}$  & $\Sexpr{lmWcoef[4]}$  & $\Sexpr{lmWcoef[5]}$  & $\Sexpr{lmWcoef[6]}$  & $\Sexpr{lmWR2}$  &  & $\Sexpr{anova(lmAbs, lmW)$F[2]}$   & $\Sexpr{anova(lmAbs, lmWSquare)$F[2]}$   \\
...
After checking our criterion, we find that it is appropriate to use more than difference scores on both examples. If we examine closer, our first model suggests using a linear model between to the two predictors and the response (p-val $\Sexpr{anova(lm, lmSquare)$Pr[2]}$). In our second model which examines the relationship between observed relationship quality of the marriage and the precieved relationship instabililty has moderate evidence of using a quadratic model for the relationship (p-val $\Sexpr{anova(lm2, lm2Square)$Pr[2]}$) and has very weak evidence for using a cubic model on the relationship (p-val $\Sexpr{anova(lm2Cube, lm2Square)$Pr[2]}$). Given this information, we will proceed with using the quadratic polynomial regression model on our second example. While we could reach for using a cubic model, in this case given the complexity of the cubic polynomial regression it is best to stick with quadratic regression which already has enough challenges.
...
Using $\verb|Plotly|$, we can look at and manipulate the predicted surface by following this link \href{https://plot.ly/~nulloa1/147/}{link}. I have included some screenshots here so you can get a feel for the convexity of the surface which will help in the interpretations of the key components in the surface. 
...
We can look atsurface slice plots in of both principal axes in Figure \ref{fig:Our_Principal_Axes}.
