...
\frametitle{Motivation}
\begin{itemize}
  \item Some methodological issues with Difference Scores
  \begin{enumerate}
    \item Difficult to identify the underlying mechanism
    \begin{itemize}
      \item Two distinct measures are turned into one entity
    \end{itemize}
    \item Problems with underlying assumptions
    \begin{itemize}
      \item Difference between two entities to the outcome variable are assumed to be symmetric
      \item Outcome is constant at all points where the two entities are equal e.g. response along $y=x$ would need to be constant
  \end{enumerate}
\end{itemize}
\end{frame}
