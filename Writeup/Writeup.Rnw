...
%------------------------------------%
\section{Intro}
Dyadic data are often used by family researchers who are interested in comparing the attitudes, behaviors, and opinions of husbands and wives, parents and children, and romantic partners towards each other. The dyadic structure can come from inter-individual reporting (e.g. comparison of husband's report of his support toward his wife to the wife's report of her support towards the husband), or it can come from intra-individual reporting (e.g. comparison of an observer's report of husband's hostility to an observer's report of the wife's hostility). \\

One of the most common methods in analyzing dyadic data is difference scores. Difference scores are used to see how well two variables/components 'fit' together. Some of the most common difference scores use the algebraic, absolute, and squared difference between the two variables as shown by Equations \ref{eq:diffscore}, \ref{eq:absdiffscore}, and \ref{eq:squarediffscore} respectively. These difference scores are often modeled to predict for an outcome variable labeled $Z$ in the equations below,
...
%------------------------------------%
\section{The Model}
The linear regression model (Eq \ref{eq:linreg}) is familiar to most researchers; it is often used in research due in part to the ease and familiarity of the interpretations of the parameters and the ease of computation. A slight tweak to simple linear models is to include two explanatory variables as in Eq \label{eq:linreg2}. Often these models also include their interactions e.g $z = a + bx + cy + dxy$. We call Eq \label{eq:linreg2} the linear difference model; this particular model does not contain an interaction.
...
\end{align}
The fitted model in matrix notation is:
\begin{align}
...
We can fit a model like this in $\verb|R|$ using the $\verb|lm()|$ function. This setup corresponds to Eq \cite{eq:quadfit} where $\verb|Z|$ is the response variable, and $\verb|X|$ and $\verb|Y|$ are the explanatory variables.
...
\end{align}
These points represent the values in our predictors that will optimize our response. Optimizing the response either means a minimum or maximum depending on the surface shape.\\
...
The purpose of principal axes is to make interpretations of the model easier by rotating the axes so that we can remove all the cross-product terms. Khuri and Cornell (1996), {\it Response Surfaces: Designs and Analyses} \cite{khuri1996response}, lays out the process of deriving the canonical equations. The canonical equations come from transforming the response system to its canonical form (this is the name of the form where the surface lives with its cross-product terms are removed); this process occurs because it represents the response centered around the stationarity points. These canonical equations are then used in finding the principal axes. In order to complete this process, new axes are created which the surface will be on. This maybe useful in some applications, but the problem is it becomes difficult to relate the response to our original variables. Essentially it becomes difficult to interpret how our original variables are affecting the response. So we focus on the process of transforming these canonical equations back to functions of the original variables i.e. transform our principal axes back to the original variables. By doing this we get the benefits of the principal axes related to our original variables.\\
...
Now that we have devloped some intuition about the model, we will look at some examples of their usage. First, we will recreate an example of the model being used in a paper then we will apply this methodology to our own data.
...
To check our understanding of how to run the model and make sure that our results are consistent with previous works, we reproduced the results in Phillips et al. (2012), {\it Congruence research in behavioral medicine: methodological review and demonstration of alternative methodology} \cite{Phillips2012}. In this paper, they were interested in seeing what created good situations in which patients followed through with their doctors orders. To study this they looked at patient-reported adherence a month after the doctor visit and its correspondence to the physician's rating of patient's health and the physician's estimate of how the patient would rate of patient's own health. We'll begin by first outlining the variables used here:
...
Presented in the paper are two different components of using polynomial regression: Confirmatory and Exploratory. The first component (confirmatory) checks whether or not it is appropriate to use polynomial regression, and the second component (exploratory) is in the name. Essentially exploratory analysis is where you would check hypothesis you have in mind, or just explore the model to see what you can learn. \\

First discussed is the use of polynomial regression in a confirmatory approach. We are looking to see whether or not we are supposed to use polynomial regression. First, we will check the assumptions of normality for all the models namely looking for points with high leverage and potential influential points. No points showed any major signs of concern although there was one point which consistently had a higher leverage and Cook's D value compared to the rest of the points in the data set. This is not a major issue, but rather something we need to keep in the back of our mind and be aware of while the analysis is continued. Next, in the paper, four criterion are presented which help decide whether using a polynomial surface is appropriate. The four criterion are:
